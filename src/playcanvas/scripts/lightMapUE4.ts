...
declare module "playcanvas" {
  interface ScriptComponent {
    LightMap?: LightMapUE4;
  }
}

interface LightMapUE4 {
  texture: pc.Asset;
  texCoord: number;
  lightmapAdd: number[];
...
const LightMapUE4ScriptName = "LightMapUE4";

class LightMapUE4 extends pc.ScriptType {
  ...
  public applyLightmapToModel(): void {
    ...
    this.entity.model.meshInstances.forEach(instance => {
      ...
      instance.material = LightMapUE4._findOrCreateExtendedMaterial(
        this,
  ...
  public removeLightmapFromModel(): void {
    ...
    this.entity.model.meshInstances.forEach(instance => {
      const originalMaterial = LightMapUE4._findOriginalMaterial(
        instance.material as pc.StandardMaterial,
      );
  ...
  private static _getHighQualityShaderCode(): string {
    return `
      ...
      {
        vec2 lightmapUv0 = getLightmapUV0($UV);
        vec2 lightmapUv1 = getLightmapUV1($UV);
...
LightMapUE4.attributes.add("texture", {
  type: "asset",
  assetType: "texture",
  title: "Texture",
});

LightMapUE4.attributes.add("texCoord", {
  type: "number",
  title: "TexCoord",
});

LightMapUE4.attributes.add("lightmapAdd", {
  type: "number",
  array: true,
  title: "Lightmap add coefficients",
});

LightMapUE4.attributes.add("lightmapScale", {
  type: "number",
  array: true,
  title: "Lightmap scale coefficients",
});

LightMapUE4.attributes.add("coordinateScaleBias", {
  type: "number",
  array: true,
  title: "Coordinate scale & bias",
});

export { LightMapUE4ScriptName, LightMapUE4 };
