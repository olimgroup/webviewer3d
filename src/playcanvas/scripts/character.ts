...
export class CharacterComponent extends pc.ScriptType {
  ...
  update(dt: number) {
    ...
    const result = from.add(nomal.mulScalar(dt).mulScalar(this._speed));
...
export class Character extends pc.Entity {
  ...
  public getColor() {
    return new pc.Color(this._material.diffuse.r, this._material.diffuse.g, this._material.diffuse.b, this._material.diffuse.a);
  }
}